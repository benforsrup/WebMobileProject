...
class ListScreen extends PureComponent {
  constructor(props) {
    ...
    this.firebaseRef = firebase
      ...
      .where("feature.properties.KMN_NAMN", "==", "Stockholm");
    ...
    this.events.addListener("navigateToMapMarker", (marker, index) =>
      this.navigateToMapMarker(marker, index)
    );
  }
  ...
  componentDidMount() {
    ...
    this.upvoteRefSubscribe = this.upvoteRef.onSnapshot(
      this.upvoteCollectionUpdate
    );
    const user = firebase.auth().currentUser;

    this.userSubsribe = firebase
      .firestore()
      .collection("users")
      .doc(user.toJSON().uid)
    ...
    geoService.getCurrentLocation().then(position => {
      if (position) {
        let url =
          "https://us-central1-badplatser-4f321.cloudfunctions.net/getClosestRequest";
        let data = {
        ...
        fetch(url, {
          ...
          response.json().then(val => this.setState({ closeMarkers: val }));
        });
      }
    });
  ...
  onCollectionUpdate = querySnapshot => {
    ...
    querySnapshot.forEach((doc) => {
      const { baddetail, feature, detail, upvotes, images } = doc.data();
      let o = {
        id: feature.id,
        location: {
        ...
        information: {
          ...
          images: images ? images : [
            "https://firebasestorage.googleapis.com/v0/b/badplatser-4f321.appspot.com/o/pic.jpg?alt=media&token=00baeaa0-19be-4fdf-9fca-b355d3efc412",
            "https://firebasestorage.googleapis.com/v0/b/badplatser-4f321.appspot.com/o/pic.jpg?alt=media&token=00baeaa0-19be-4fdf-9fca-b355d3efc412",
            "https://firebasestorage.googleapis.com/v0/b/badplatser-4f321.appspot.com/o/pic.jpg?alt=media&token=00baeaa0-19be-4fdf-9fca-b355d3efc412"
          ]
        },
  ...
  renderSearchList = () => {
    ...
    filteredBadplatser = filteredBadplatser.filter(plats => {
      ...
      return platsnamn.indexOf(this.state.search.toLowerCase()) !== -1;
    });
    return (
      <FlatList
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.container}>
          ...
          >
            ...
            >
              <Text style={{ fontSize: 30, fontFamily: "ProductSans-Regular" }}>
                Sök efter en badplats! 🏊‍
              </Text>
            </View>
            ...
            >
              <SearchBar
                ...
                cancelButtonProps={{
                  buttonTextStyle: { fontFamily: "ProductSans-Regular" }
                }}
                inputStyle={{ fontFamily: "ProductSans-Regular" }}
                inputContainerStyle={{ backgroundColor: "white" }}
                containerStyle={[
                  styles.searchBarShadow,
                  { width: wp(90), backgroundColor: "transparent" }
                ]}
